...
def char_poly_coefficients(A):
    ...
    if n == 1:
        ...
        raise NotImplementedError("Characteristic polynomial implemented only for 1x1, 2x2 and 3x3 matrices.")
...
def eigenvalues_eigenvectors(A):
    ...
    for lam in lambdas:
        ...
        for i in range(size):
            ...
            for j in range(size):
                if i == j:
                    row.append(A[i][j] - lam)
                else:
                    row.append(A[i][j])
            matrix.append(row)
        # Solve (A - lambda*I)v = 0
        try:
            # Since it's homogeneous, find a non-trivial solution
            # We can use Gaussian elimination to reduce the matrix
            la = LinearAlgebra()
            # Append a zero vector for constants
            # To find a free variable, we can set one variable to 1 and solve
            # For simplicity, we'll handle 2x2 and 3x3 matrices
            if size == 1:
                eigenvectors.append([1])
            elif size == 2:
                # Assume the first variable is free (set to 1)
                # Then solve for the second variable
                # Equation: a*x + b*y = 0
                a, b = matrix[0]
                if b != 0:
                    y = -a / b
                    ...
                    # a*x = 0 => x = 0, y can be 1
                    eigenvectors.append([0, 1])
                else:
                    # Both coefficients are zero, any vector is an eigenvector
                    eigenvectors.append([1, 0])
            elif size == 3:
                # More complex; set the first variable to 1 and solve the next two
                # If inconsistent, set the second variable to 1, etc.
                # Here, we use Gaussian elimination to find at least one solution
                # This is a simplified approach and may not cover all cases
                try:
                    solutions = la.gaussian_elimination(matrix, [0, 0, 0])
                    eigenvectors.append(solutions)
                except:
                    # If Gaussian elimination fails (e.g., infinite solutions), set a free variable
                    eigenvectors.append([1, 0, 0])
            else:
                eigenvectors.append(None)  # Not implemented
        except Exception as e:
            eigenvectors.append(None)  # Could not find eigenvector

    return lambdas, eigenvectors  # Now returns eigenvalues and their corresponding eigenvectors
...
class MathCalculatorApp:
    ...
    def create_eigen_tab(self):
        ...
        # Solve button
        solve_button = ttk.Button(frame, text="Find Eigenvalues & Eigenvectors", command=self.find_eigen)
        solve_button.pack(pady=10)

        # Result display
        self.eigen_result = tk.Text(frame, height=15, width=80, state='disabled')
    ...
    def find_eigen(self):
        try:
            ...
            lambdas, eigenvectors = eigenvalues_eigenvectors(A)
            # Prepare result string
            result_str = "Eigenvalues and Eigenvectors:\n"
            for idx, lam in enumerate(lambdas):
                lam = round(lam, 6)
                vec = eigenvectors[idx] if eigenvectors[idx] else "Not computed"
                if vec != "Not computed":
                    vec = [round(x, 6) for x in vec]
                result_str += f"λ{idx+1} = {lam}\n"
                result_str += f"v{idx+1} = {vec}\n\n"
        except Exception as e:
            result_str = f"Error: {e}"
    ...
    def display_eigen_result(self, lambdas, eigenvectors):
        result_str = "Eigenvalues and Eigenvectors:\n"
        for idx, lam in enumerate(lambdas):
            lam = round(lam, 6)
            vec = eigenvectors[idx] if eigenvectors[idx] else "Not computed"
